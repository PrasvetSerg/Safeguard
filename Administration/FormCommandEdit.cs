...
{
    //ТИП FEDIT4+
    public partial class FormCommandEdit : DevExpress.XtraEditors.XtraForm
    {
        ActionType actionType;
        int recordId;
        int parentRecordId;

        public FormCommandEdit(ActionType actionType, int recordId, int parentRecordId)
        {
        ...
        private void UpdateDataOnForm()
        {
            ...
            if (this.recordId > 0)
            {
                ...
                DataTable dataTable = DBConnection.SqlQuery.ReadData(this, string.Format("dtr_acl_command_get 'by_acl_command_id',{0}", this.RecordId));

                if (dataTable != null && dataTable.Rows.Count > 0)
                {
                    this.idTextEdit.Text = dataTable.Rows[0]["acl_command_id"].ToString();
                    this.parentRecordId = (int)dataTable.Rows[0]["acl_procedure_id"];
                    this.nameTextEdit.Text = dataTable.Rows[0]["command_name"].ToString();
                    this.displayNameTextEdit.Text = dataTable.Rows[0]["command_display_name"].ToString();
        ...
        private void ActivateControl()
        {
            ...
            {
                this.Text = "Регистрация новой команды";
            ...
            {
                this.Text = "Редактирование информации о команды";
        ...
        private bool IsInputedDataValid()
        {
            ...
            */
            ...
            {
                isInputedDataValid = false;
                DevExpress.XtraEditors.XtraMessageBox.Show(this, "Заполните поле команда", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            ...
            {
                //запрос всех записай
                string query = string.Format("dtr_acl_command_get 'all'"
                    );
                ...
                if (dataTable != null)// && dataTable.Rows.Count > 0)
                {
                    string filterExprssion = string.Format("expiration_date IS NULL AND (acl_command_name='{0}')"
        ...
        private bool SaveData()
        {
            ...
            string query = string.Concat("dtr_acl_command_set"
                , " @command=" + DBConnection.SqlQuery.ActionTypeToDbString(this.actionType)
                , ",@acl_command_id=" + this.RecordId
                , ",@acl_procedure_id=" + this.parentRecordId
                , ",@command_name=" + DBConnection.SqlQuery.StrToDbString(this.nameTextEdit.Text.Trim())
                , ",@command_display_name=" + DBConnection.SqlQuery.StrToDbString(this.displayNameTextEdit.Text.Trim())
            ...
            try
            {
                ...
                if (dataTable.Rows.Count == 1)
                {
                    ...
                    this.recordId = (int)dataTable.Rows[0]["acl_command_id"];
                }
            }
