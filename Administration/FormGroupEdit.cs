...
{
    //ТИП FEDIT4+
    public partial class FormGroupEdit : DevExpress.XtraEditors.XtraForm
    {
        ActionType actionType;
        int recordId;

        public FormGroupEdit(ActionType actionType, int recordId)
        {
        ...
        private void UpdateDataOnForm()
        {
            ...
            if (this.recordId > 0)
            {
                ...
                DataTable dataTable = DBConnection.SqlQuery.ReadData(this, string.Format("dtr_acl_group_get 'by_acl_group_id',{0}", this.RecordId));

                if (dataTable != null && dataTable.Rows.Count > 0)
                {
                    this.idTextEdit.Text = dataTable.Rows[0]["acl_group_id"].ToString();
                    this.nameTextEdit.Text = dataTable.Rows[0]["group_name"].ToString();                    
                }
            }
        ...
        private void ActivateControl()
        {
            ...
            {
                this.Text = "Регистрация новой группы";
        ...
        private bool IsInputedDataValid()
        {
            ...
            */
            ...
            {
                isInputedDataValid = false;
                DevExpress.XtraEditors.XtraMessageBox.Show(this, "Заполните поле наименования группы", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            /*
            ...
            {
                //запрос всех записай
                string query = string.Format("dtr_acl_group_get 'all'"
                    );
                ...
                if (dataTable != null)// && dataTable.Rows.Count > 0)
                {
                    string filterExprssion = string.Format("expiration_date IS NULL AND (acl_group_name='{0}')"
        ...
        private bool SaveData()
        {
            ...
            string query = string.Concat("dtr_acl_group_set"
                , " @command=" + DBConnection.SqlQuery.ActionTypeToDbString(this.actionType)
                , ",@acl_group_id=" + this.RecordId
                , ",@group_name=" + DBConnection.SqlQuery.StrToDbString(this.nameTextEdit.Text.Trim())
                );
            ...
            try
            {
                ...
                if (dataTable.Rows.Count == 1)
                {
                    ...
                    this.recordId = (int)dataTable.Rows[0]["acl_group_id"];
                }
            }
