...
{
    //ТИП CLIST4++
    public partial class UcAclList : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        {
            DataTable dataTable = DBConnection.SqlQuery.ReadData(null, "dtr_acl_group_get 'all_view'");
            if (dataTable != null && dataTable.Rows.Count > 0)
            {
                //this.groupRepositoryItemLookUpEdit.Properties.DataSource = dataTable;
                //this.groupRepositoryItemLookUpEdit.Properties.DisplayMember = "group_name";
                //this.groupRepositoryItemLookUpEdit.Properties.ValueMember = "acl_group_id";
                this.groupRepositoryItemLookUpEdit.DataSource = dataTable;
                this.groupRepositoryItemLookUpEdit.DisplayMember = "group_name";
                this.groupRepositoryItemLookUpEdit.ValueMember = "acl_group_id";
            }
        }

        int parentId;
        public void ShowList(DisplayMode displayMode, int idForSelect = -1, int parentId = 1)
        {
            this.displayMode = displayMode;
            this.parentId = parentId;

            if (this.groupRepositoryItemLookUpEdit.DataSource == null)
            {
                InitData();
                this.groupBarEditItem.EditValue = parentId;
            }
            ...
            string query = string.Format("dtr_acl_get '{0}',{1}"
                , mode
                , parentId
            ...
            if (idForSelect > 0)
            {
                SelectRow(idForSelect);

                if (this.SelectedRecordId != idForSelect) //действительное не равно желаемому
                {
        ...
        {
            using (FormGroupEdit formEdit = new FormGroupEdit(actionType, id))
            {
        ...
        */
        ...
        private void repositoryItemCheckEdit1_Click(object sender, EventArgs e)
        {
            if (this.SelectedRecordId > 0)
            {
                if (this.SelectedRecord["acl_id"] == DBNull.Value)
                {
                    AddPermission();
                }
                else
            ...
            //this.repositoryItemCheckEdit1.FullFocusRect;
        }
        ...
        private void AddPermission()
        {
            ...
            try
            {
                DataTable dataTable = DBConnection.SqlQuery.RunTableQuery(string.Concat("dtr_acl_set"
                    , " @command=" + DBConnection.SqlQuery.ActionTypeToDbString(ServiceTypes.ActionType.Add)
                    , ",@acl_command_id=" + this.SelectedRecordId
                    , ",@acl_group_id=" + this.parentId
                    ));
            ...
            if (isSave == true)
            {
                //ShowList(this.displayMode, this.parentId);
        ...
        private void DeletePermission()
        {
            ...
            try
            {
                DataTable dataTable = DBConnection.SqlQuery.RunTableQuery(string.Concat("dtr_acl_set"
                    ...
                    , ",@acl_id=" + this.SelectedRecord["acl_id"].ToString()
                    ));
                if (dataTable.Rows.Count == 1)
                {
                    isSave = true;
        ...
        {
            ShowList(DisplayMode.ByGroupId, parentId: (int)this.groupBarEditItem.EditValue);            
        }
    }
}
