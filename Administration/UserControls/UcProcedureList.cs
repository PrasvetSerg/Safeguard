...
{
    //ТИП CLIST4++
    public partial class UcProcedureList : DevExpress.XtraEditors.XtraUserControl
    {
        readonly string idFieldName = "acl_procedure_id";
        ...
        public void ShowList(DisplayMode displayMode, int id)
        {
            ...
            string query = string.Format("dtr_acl_procedure_get '{0}'"//,{1}"
                , mode
        ...
        {
            using (FormProcedureEdit formEdit = new FormProcedureEdit(actionType, id))
            {
        ...
        */
        ...
        private void importBarButtonItem_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                DataTable dataTable = DBConnection.SqlQuery.RunTableQuery(string.Concat("dtr_acl_procedure_set"
                    , " @command='import'"
                    ));
                if (dataTable.Rows.Count > 0)
                {
                    UpdateData(this.lastQuery);
                    DevExpress.XtraEditors.XtraMessageBox.Show(this, "Выполнено успешно", "Актуализация", MessageBoxButtons.OK);
                }
            }
