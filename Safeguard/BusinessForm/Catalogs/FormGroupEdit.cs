...
namespace Safeguard.BusinessForm.Catalogs
{
    public partial class FormGroupEdit : DevExpress.XtraEditors.XtraForm
    {
        ...
        public FormGroupEdit(ServiceTypes.ActionType actionType = ActionType.None, int recordId = -1, int parentId = -1, int relativePosition = 999)
        {
        ...
        private void UpdateDataOnForm()
        {
            ...
            if (this.recordId > 0)
            {
                ...
                DataTable dataTable = DBConnection.SqlQuery.ReadData(this, string.Format("dtr_group_get 'by_group_id',{0}", this.RecordId));

                if (dataTable != null && dataTable.Rows.Count > 0)
                {
                    this.idTextEdit.Text = dataTable.Rows[0]["group_id"].ToString();
                    //this.parentRecordId = (dataTable.Rows[0]["parent_group_id"] == DBNull.Value ? -1 : (int)dataTable.Rows[0]["parent_group_id"]);
        ...
        private bool SaveData()
        {
            ...
            string query = string.Concat("dtr_group_set"
                , " @command=" + DBConnection.SqlQuery.ActionTypeToDbString(this.actionType)
                , ",@group_id=" + this.RecordId
                //, ",@parent_group_id=" + DBConnection.SqlQuery.IdToDbId(this.parentRecordId)
            ...
            try
            {
                ...
                if (dataTable.Rows.Count == 1)
                {
                    ...
                    this.recordId = (int)dataTable.Rows[0]["group_id"];
                }
            }
