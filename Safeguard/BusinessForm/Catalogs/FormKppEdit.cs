...
{
    public partial class FormKppEdit : DevExpress.XtraEditors.XtraForm
    {
        ...
        public FormKppEdit(ServiceTypes.ActionType actionType = ActionType.None, int recordId = -1, int parentId = -1, int relativePosition = 999)
        {
        ...
        private void UpdateDataOnForm()
        {
            ...
            if (this.recordId > 0)
            {
                ...
                DataTable dataTable = DBConnection.SqlQuery.ReadData(this, string.Format("dtr_kpp_get 'by_kpp_id',{0}", this.RecordId));

                if (dataTable != null && dataTable.Rows.Count > 0)
                {
                    this.idTextEdit.Text = dataTable.Rows[0]["kpp_id"].ToString();
                    //this.parentRecordId = (dataTable.Rows[0]["parent_kpp_id"] == DBNull.Value ? -1 : (int)dataTable.Rows[0]["parent_kpp_id"]);
                    this.nameTextEdit.Text = dataTable.Rows[0]["kpp_name"].ToString();
        ...
        private void ActivateControl()
        {
            ...
            {
                this.Text = "Регистрация нового места задержания";
            ...
            {
                this.Text = "Редактирование информации о месте задержания";
        ...
        private bool SaveData()
        {
            ...
            string query = string.Concat("dtr_kpp_set"
                , " @command=" + DBConnection.SqlQuery.ActionTypeToDbString(this.actionType)
                , ",@kpp_id=" + this.RecordId
                //, ",@parent_kpp_id=" + DBConnection.SqlQuery.IdToDbId(this.parentRecordId)
                , ",@kpp_name=" + DBConnection.SqlQuery.StrToDbString(this.nameTextEdit.Text.Trim())
            ...
            try
            {
                ...
                if (dataTable.Rows.Count == 1)
                {
                    ...
                    this.recordId = (int)dataTable.Rows[0]["kpp_id"];
                }
            }
