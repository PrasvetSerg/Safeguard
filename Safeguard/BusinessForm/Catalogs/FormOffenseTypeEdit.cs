...
{
    public partial class FormOffenseTypeEdit : DevExpress.XtraEditors.XtraForm
    {
        ...
        public FormOffenseTypeEdit(ServiceTypes.ActionType actionType = ActionType.None, int recordId = -1, int parentId = -1, int relativePosition = 999)
        {
        ...
        private void UpdateDataOnForm()
        {
            ...
            if (this.recordId > 0)
            {
                ...
                DataTable dataTable = DBConnection.SqlQuery.ReadData(this, string.Format("dtr_offense_type_get 'by_offense_type_id',{0}", this.RecordId));

                if (dataTable != null && dataTable.Rows.Count > 0)
                {
                    this.idTextEdit.Text = dataTable.Rows[0]["offense_type_id"].ToString();
                    //this.parentRecordId = (dataTable.Rows[0]["parent_offense_type_id"] == DBNull.Value ? -1 : (int)dataTable.Rows[0]["parent_offense_type_id"]);
                    this.nameTextEdit.Text = dataTable.Rows[0]["offense_type_name"].ToString();
                    this.commentTextEdit.Text = dataTable.Rows[0]["comment"].ToString();
                    //this.sequenceTextEdit.Text = dataTable.Rows[0]["sequence"].ToString();
                    this.reasonMemoEdit.Lines = BusinessObjects.AnswerVariants.RestoreFromXmlString(dataTable.Rows[0]["reason_variants"].ToString()).List.ToArray();
                    this.penaltyMemoEdit.Lines = BusinessObjects.AnswerVariants.RestoreFromXmlString(dataTable.Rows[0]["penalty_variants"].ToString()).List.ToArray();
                }
            }
            else
            {
        ...
        private void ActivateControl()
        {
            ...
            {
                this.Text = "Регистрация нового вида правонарушения";
            ...
            {
                this.Text = "Редактирование информации о виде правонарушения";
        ...
        private bool SaveData()
        {
            ...
            string query = string.Concat("dtr_offense_type_set"
                , " @command=" + DBConnection.SqlQuery.ActionTypeToDbString(this.actionType)
                , ",@offense_type_id=" + this.RecordId
                //, ",@parent_offense_type_id=" + DBConnection.SqlQuery.IdToDbId(this.parentRecordId)
                , ",@offense_type_name=" + DBConnection.SqlQuery.StrToDbString(this.nameTextEdit.Text.Trim())
                , ",@comment=" + DBConnection.SqlQuery.StrToDbString(this.commentTextEdit.Text.Trim())
                //, ",@sequence=" + this.sequenceTextEdit.Text.ToString()
                , ",@reason_variants=" + DBConnection.SqlQuery.StrToDbString(BusinessObjects.AnswerVariants.SaveToXmlString(new BusinessObjects.AnswerVariants(new List<string>(this.reasonMemoEdit.Lines))))
                , ",@penalty_variants=" + DBConnection.SqlQuery.StrToDbString(BusinessObjects.AnswerVariants.SaveToXmlString(new BusinessObjects.AnswerVariants(new List<string>(this.penaltyMemoEdit.Lines))))
                );
            ...
            try
            {
                ...
                if (dataTable.Rows.Count == 1)
                {
                    ...
                    this.recordId = (int)dataTable.Rows[0]["offense_type_id"];
                }
            }
