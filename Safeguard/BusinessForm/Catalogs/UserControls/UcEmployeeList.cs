...
namespace Safeguard.BusinessForm.Catalogs.UserControls
{
    ...
    {
        readonly string idFieldName = "employee_id";
        ...
        private void ShowAbonent(int abonentTreeId)
        {
            ...
            if (this.aResult == null || this.aResult.IsCompleted == true)
            {
                ...
                string query = string.Format("dtr_employee_get '{0}',{1}"
                    , mode
                    , abonentTreeId
                    );
        ...
        {
            using (FormEmployeeEdit formEdit = new FormEmployeeEdit(actionType, abonentCardId, bookTreeId))
            {
        ...
        private bool IsNeedChangeObject(int oldObjectId, int newObjectId)
        {
            ...
            if(isNeed == true)
            {
                string query = string.Format(string.Format("dtr_employee_get 'by_employee_parents',{0}", newObjectId));
                ...
                if (dataTable != null)
                {
                    ...
                    {
                        if(oldObjectId == (int)row["employee_id"])
                        {
                            isNeed = false;
                            break;
                        }
        ...
        {
            ShowRowMenu(sender, e);
        }
        private void ShowRowMenu(object sender, DevExpress.XtraGrid.Views.Grid.PopupMenuShowingEventArgs e)
        {
            DevExpress.XtraGrid.Views.Grid.GridView _gridView = (DevExpress.XtraGrid.Views.Grid.GridView)sender;

            if (e.MenuType == DevExpress.XtraGrid.Views.Grid.GridMenuType.Column)
            {
                //_gridView.AddCustomAdjustmentMenuItems(e.Menu);
            }
            else if (e.MenuType == DevExpress.XtraGrid.Views.Grid.GridMenuType.Row)
            {
                if (_gridView.IsDataRow(_gridView.FocusedRowHandle))
                {
                    Point _screenPoint = this.gridControl.PointToScreen(e.Point);
                    this.popupMenu.ShowPopup(_screenPoint);
                }
            }
        }
    }
