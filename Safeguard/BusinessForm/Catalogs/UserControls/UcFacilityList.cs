...
namespace Safeguard.BusinessForm.Catalogs.UserControls
{
    ...
    public partial class UcFacilityList : DevExpress.XtraEditors.XtraUserControl
    {
        readonly string idFieldName = "facility_id";
        ...
        public void ShowList(DisplayMode displayMode, int bookTreeId,int abonentCardIdForSelect = -1)
        {
            ...
            if (this.aResult == null || this.aResult.IsCompleted == true)
            {
                ...
                switch (displayMode)
                {
                    case DisplayMode.All:
                        mode = "all_view";
                        break;
                    case DisplayMode.ByObjectId:
                        mode = "by_department_id_view";
                        break;
                }
                string query = string.Format("dtr_facility_get '{0}',{1}"
                    , mode
                    , abonentTreeId
                    );
        ...
        {
            using (FormFacilityEdit formEdit = new FormFacilityEdit(actionType, abonentCardId, bookTreeId))
            {
        ...
        private bool IsNeedChangeObject(int oldObjectId, int newObjectId)
        {
            ...
            if(isNeed == true)
            {
                string query = string.Format(string.Format("dtr_facility_get 'by_facility_parents',{0}", newObjectId));
                ...
                if (dataTable != null)
                {
                    ...
                    {
                        if(oldObjectId == (int)row["facility_id"])
                        {
                            isNeed = false;
                            break;
                        }
