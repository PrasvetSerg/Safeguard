...
namespace Safeguard.BusinessForm.Catalogs.UserControls
{
    ...
    public partial class UcGroupList : DevExpress.XtraEditors.XtraUserControl
    {
        readonly string idFieldName = "group_id";
        ...
        private void ShowAbonent(int abonentTreeId)
        {
            ...
            if (this.aResult == null || this.aResult.IsCompleted == true)
            {
                ...
                string query = string.Format("dtr_group_get '{0}',{1}"
                    , mode
                    , abonentTreeId
                    );
        ...
        {
            using (FormGroupEdit formEdit = new FormGroupEdit(actionType, abonentCardId, bookTreeId))
            {
        ...
        private bool IsNeedChangeObject(int oldObjectId, int newObjectId)
        {
            ...
            if(isNeed == true)
            {
                string query = string.Format(string.Format("dtr_group_get 'by_group_parents',{0}", newObjectId));
                ...
                if (dataTable != null)
                {
                    ...
                    {
                        if(oldObjectId == (int)row["group_id"])
                        {
                            isNeed = false;
                            break;
                        }
