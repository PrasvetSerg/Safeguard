...
namespace Safeguard.BusinessForm.Catalogs.UserControls
{
    ...
    public partial class UcKppList : DevExpress.XtraEditors.XtraUserControl
    {
        readonly string idFieldName = "kpp_id";
        ...
        private void ShowAbonent(int abonentTreeId)
        {
            ...
            if (this.aResult == null || this.aResult.IsCompleted == true)
            {
                ...
                string query = string.Format("dtr_kpp_get '{0}',{1}"
                    , mode
                    , abonentTreeId
                    );
        ...
        {
            using (FormKppEdit formEdit = new FormKppEdit(actionType, abonentCardId, bookTreeId))
            {
        ...
        private bool IsNeedChangeObject(int oldObjectId, int newObjectId)
        {
            ...
            if(isNeed == true)
            {
                string query = string.Format(string.Format("dtr_kpp_get 'by_kpp_parents',{0}", newObjectId));
