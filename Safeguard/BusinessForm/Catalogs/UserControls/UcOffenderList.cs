...
namespace Safeguard.BusinessForm.Catalogs.UserControls
{
    ...
    public partial class UcOffenderList : DevExpress.XtraEditors.XtraUserControl
    {
        readonly string idFieldName = "offender_id";
        ...
        private void ShowAbonent(int abonentTreeId)
        {
            ...
            if (this.aResult == null || this.aResult.IsCompleted == true)
            {
                ...
                string query = string.Format("dtr_offender_get '{0}',{1}"
                    , mode
                    , abonentTreeId
                    );
        ...
        {
            using (FormOffenderEdit formEdit = new FormOffenderEdit(actionType, abonentCardId, bookTreeId))
            {
        ...
        private bool IsNeedChangeObject(int oldObjectId, int newObjectId)
        {
            ...
            if(isNeed == true)
            {
                string query = string.Format(string.Format("dtr_offender_get 'by_offender_parents',{0}", newObjectId));
                ...
                if (dataTable != null)
                {
                    ...
                    {
                        if(oldObjectId == (int)row["offender_id"])
                        {
                            isNeed = false;
                            break;
                        }
