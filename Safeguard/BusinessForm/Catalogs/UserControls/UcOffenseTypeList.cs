...
namespace Safeguard.BusinessForm.Catalogs.UserControls
{
    ...
    public partial class UcOffenseTypeList : DevExpress.XtraEditors.XtraUserControl
    {
        readonly string idFieldName = "offense_type_id";
        ...
        private void ShowAbonent(int abonentTreeId)
        {
            ...
            if (this.aResult == null || this.aResult.IsCompleted == true)
            {
                ...
                string query = string.Format("dtr_offense_type_get '{0}',{1}"
                    , mode
                    , abonentTreeId
                    );
        ...
        {
            using (FormOffenseTypeEdit formEdit = new FormOffenseTypeEdit(actionType, abonentCardId, bookTreeId))
            {
        ...
        private bool IsNeedChangeObject(int oldObjectId, int newObjectId)
        {
            ...
            if(isNeed == true)
            {
                string query = string.Format(string.Format("dtr_offense_type_get 'by_offense_type_parents',{0}", newObjectId));
                ...
                if (dataTable != null)
                {
                    ...
                    {
                        if(oldObjectId == (int)row["offense_type_id"])
                        {
                            isNeed = false;
                            break;
                        }
