...
namespace Safeguard.BusinessForm.Catalogs.UserControls
{
    ...
    public partial class UcPostList : DevExpress.XtraEditors.XtraUserControl
    {
        readonly string idFieldName = "post_id";
        ...
        private void ShowAbonent(int abonentTreeId)
        {
            ...
            if (this.aResult == null || this.aResult.IsCompleted == true)
            {
                ...
                string query = string.Format("dtr_post_get '{0}',{1}"
                    , mode
                    , abonentTreeId
                    );
        ...
        {
            using (FormPostEdit formEdit = new FormPostEdit(actionType, abonentCardId, bookTreeId))
            {
        ...
        private bool IsNeedChangeObject(int oldObjectId, int newObjectId)
        {
            ...
            if(isNeed == true)
            {
                string query = string.Format(string.Format("dtr_post_get 'by_post_parents',{0}", newObjectId));
                ...
                if (dataTable != null)
                {
                    ...
                    {
                        if(oldObjectId == (int)row["post_id"])
                        {
                            isNeed = false;
                            break;
                        }
