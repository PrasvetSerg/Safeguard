...
namespace Safeguard.BusinessForm.Journals.UserControls
{
    ...
    public partial class UcGroupEmployeeList : DevExpress.XtraEditors.XtraUserControl
    {
        readonly string idFieldName = "group_employee_id";
        ...
        private void ShowAbonent(int abonentTreeId)
        {
            ...
            if (this.aResult == null || this.aResult.IsCompleted == true)
            {
                ...
                string query = string.Format("dtr_group_employee_get '{0}',{1}"
                    , mode
                    , abonentTreeId
                    );
        ...
        {
            using (FormGroupEmployeeEdit formEdit = new FormGroupEmployeeEdit(actionType, abonentCardId, bookTreeId))
            {
        ...
        private bool IsNeedChangeObject(int oldObjectId, int newObjectId)
        {
            ...
            if(isNeed == true)
            {
                string query = string.Format(string.Format("dtr_group_employee_get 'by_group_employee_parents',{0}", newObjectId));
                ...
                if (dataTable != null)
                {
                    ...
                    {
                        if(oldObjectId == (int)row["group_employee_id"])
                        {
                            isNeed = false;
                            break;
                        }
