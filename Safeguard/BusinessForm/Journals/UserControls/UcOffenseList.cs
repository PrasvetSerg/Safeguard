...
namespace Safeguard.BusinessForm.Journals.UserControls
{
    ...
    public partial class UcOffenseList : DevExpress.XtraEditors.XtraUserControl
    {
        readonly string idFieldName = "offense_id";
        ...
        private void ShowAbonent(int abonentTreeId)
        {
            ...
            if (this.aResult == null || this.aResult.IsCompleted == true)
            {
                ...
                string query = string.Format("dtr_offense_get '{0}',{1}"
                    , mode
                    , abonentTreeId
                    );
        ...
        {
            using (FormOffenseEdit formEdit = new FormOffenseEdit(actionType, abonentCardId, bookTreeId))
            {
        ...
        private bool IsNeedChangeObject(int oldObjectId, int newObjectId)
        {
            ...
            if(isNeed == true)
            {
                string query = string.Format(string.Format("dtr_offense_get 'by_offense_parents',{0}", newObjectId));
                ...
                if (dataTable != null)
                {
                    ...
                    {
                        if(oldObjectId == (int)row["offense_id"])
                        {
                            isNeed = false;
                            break;
                        }
